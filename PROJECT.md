...
- **OpenRGB devices (Gigabyte GPU, Razer keyboard and mouse)** (`openrgb_output.py`): the OpenRGB Windows service (auto-start, SDK on 127.0.0.1:6742) must be running. List devices in `config.json` > `openrgb.devices` (exact OpenRGB names). These devices in Direct mode just hold the per-LED colors last sent, so the bridge keeps a **base look** and sends base x brightness.
  - Just set your colors in OpenRGB (at full brightness). The bridge writes only when you change brightness, power or a preset, never on a timer. At that moment, if a device no longer shows what the bridge last wrote, it adopts the device's current colors as the new look (log: "was changed in OpenRGB") and scales them. So colors you pick stay until the next brightness change, then get dimmed. The look is saved in `bridge/base_colors.json` (git-ignored). `POST /capture` / the test page button does the same adoption on demand.
  - Adoption never mistakes the bridge's own output or an OpenRGB re-detection (which resets colors and changes controller IDs) for an edit. After a bridge restart the saved look wins, so a look you set while the bridge was down needs one capture.
  - Rescans for missing devices (e.g. the wired Cobra entry when only the wireless one is present) write nothing to working devices. An earlier version re-sent the saved look every 60 s and overwrote colors set in OpenRGB; iCUE re-checks no longer touch the OpenRGB output either.
